	...
	*/
		...
		if(!$result = $db->query($sql)){
			...
			while ($row = $db->fetch_array($result)) {
					...
					if($this->is_full_songcircle($row['participants'], $this->num_of_parts($row['songcircle_id'])) && $this->is_not_registered($row['songcircle_id'])){
						...
						$output.= "<input type=\"hidden\" name=\"date_of_songcircle\" value=\"".date("l, F jS, Y \\a\\t g:i A", strtotime($row['date_of_songcircle']))."\">";
	...
	function register($songcircle_id, $user_id, $username, $songcircle_name, $date_of_songcircle){
		...
		if(!$result = $db->query($sql)){
			...
			return $this->message = "You successfully registered for \"{$songcircle_name}\" on ".$date_of_songcircle.". Check your inbox for more information.";
		}
	}
	...
	public function unregister($songcircle_id, $user_id, $songcircle_name, $date_of_songcircle){
		...
		if(!$result = $db->query($sql)){
			...
			return $this->message = "You have unregistered from \"{$songcircle_name}\" on ".$date_of_songcircle.".";
		}
	}
	...
	function user_timezone($date_of_songcircle){
